...
class _FakeHomeRepo implements HomeRepo {
  ...
  HomeBlock<MachinesSummary> machinesResult = const HomeBlock<MachinesSummary>.ready(
    MachinesSummary(total: 1),
  );
  HomeBlock<TransfersSummary> transfersResult = const HomeBlock<TransfersSummary>.ready(
    TransfersSummary(pendingForMe: 2),
  );
  HomeBlock<MerchantsSummary> merchantsResult = const HomeBlock<MerchantsSummary>.ready(
    MerchantsSummary(total: 3),
  );
  HomeBlock<ViolationsSummary> violationsResult = const HomeBlock<ViolationsSummary>.ready(
    ViolationsSummary(open: 4),
  );
  HomeBlock<MaintenanceSummary> maintenanceResult = const HomeBlock<MaintenanceSummary>.ready(
    MaintenanceSummary(open: 5),
  );
  HomeBlock<FinanceSummary> financeResult = HomeBlock<FinanceSummary>.ready(
    FinanceSummary(
      period: FinancePeriod(from: DateTime(2026, 1, 1), to: DateTime(2026, 1, 31)),
      income: 100,
      expense: 40,
      net: 60,
      incomeCount: 1,
  ...
  HomeBlock<BudgetStatusList> budgetsResult = HomeBlock<BudgetStatusList>.ready(
    BudgetStatusList(asOf: DateTime(2026), budgets: const <FinanceBudgetStatus>[]),
  );
...
void main() {
  ...
  group('permission-driven block visibility', () {
    test('a Director-like permission set fetches and renders every block', () async {
    ...
    test('a representative with only machines/transfers read sees only those two', () async {
      final _FakeHomeRepo repo = _FakeHomeRepo();
      final HomeDashboardCubit cubit = HomeDashboardCubit(
        repo: repo,
        permissionService: await _permissionService(_representativePermissions),
      );
    ...
    test('a role with no read permissions at all loads to an all-null, crash-free state', () async {
      final _FakeHomeRepo repo = _FakeHomeRepo();
      final HomeDashboardCubit cubit = HomeDashboardCubit(
        repo: repo,
  ...
  group('partial-failure handling', () {
    ...
    test('retrying one block only re-fetches that block and leaves the rest untouched', () async {
      final _FakeHomeRepo repo = _FakeHomeRepo()
      ...
      expect((cubit.state as HomeDashboardLoaded).violations?.availability, HomeBlockAvailability.error);

      repo.violationsResult = const HomeBlock<ViolationsSummary>.ready(ViolationsSummary(open: 0));
      await cubit.retry(HomeBlockKind.violations);
      final HomeDashboardLoaded afterRetry = cubit.state as HomeDashboardLoaded;
